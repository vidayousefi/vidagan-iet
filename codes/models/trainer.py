...
class Trainer(object):
    ...
    def fit(self, train_loader, val_loader, epochs=5):
        ...
        for ep in range(self.epoch, epochs + 1):
            ...
            self._fit_coders(train_loader, val_loader, metrics, ep, epochs)
            end_time = time.perf_counter()
            total_coder_time += end_time - start_time
            print(
                f"FPS: {len(train_loader) * (ep - start_epoch + 1) * batch_size / total_coder_time:.2f}"
            )

            self.validate(metrics, val_loader)
    ...
    def _fit_coders(self, dataloader, val_loader, metrics, epoch, epochs):
        self.model.train()
        ...
        for idx, cover in enumerate(tqdm(dataloader)):
            ...
            (encoder_mse + (decoder_loss) * self.ratio).backward()
            ...
            if self.dev_mode and (idx + 1) % 1000 == 0:
                self.validate(metrics, val_loader)
                self.model.train()
